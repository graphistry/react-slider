...
describe('Slider', () => {
  ...
  it('should render a Slider with an updated default value correctly', () => {
    ...
    do {
      ...
      const sliderWithDefaultValue = mount(<Slider defaultValue={defaultValue} />);
      expect(sliderWithDefaultValue.state('value')).toBe(defaultValue);
      expect(sliderWithDefaultValue
             .find('.rc-slider-handle').get(0)
             .style.cssText).toMatch(sliderLeftStyle);

      const trackStyle = sliderWithDefaultValue
              .find('.rc-slider-track').get(0).style.cssText;
      expect(trackStyle).toMatch(trackLeftStyle);
      expect(trackStyle).toMatch(trackWidthStyle);
